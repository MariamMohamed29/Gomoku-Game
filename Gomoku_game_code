...
bool check_win() {
    for(int i=0;i<N;i++){
        for(int j=4;j<M;j++){
            if(grid[i][j]==grid[i][j-1]&&grid[i][j-1]==grid[i][j-2]&&grid[i][j-2]==grid[i][j-3]&&grid[i][j-3]==grid[i][j-4]&&grid[i][j]!='.')
                return 1;
        }
    }
     for(int j=0;j<M;j++){
        for(int i=4;i<N;i++){
            if(grid[i][j]==grid[i-1][j]&&grid[i-1][j]==grid[i-2][j]&&grid[i-2][j]==grid[i-3][j]&&grid[i-3][j]==grid[i-4][j]&&grid[i][j]!='.')
                return 1;
        }
    }
        for (int j= 0; j < 5; j++)
...
for(int i=0;i<N;i++){
        ...
        for (int j= 0; j < 5; j++)
    {
        int i = 4;
        while (i < N && j < M) {
            if (grid[i][j] !=mark || grid[i - 1][j + 1]!=mark || grid[i - 2][j + 2] !=mark || grid[i - 3][j + 3]!=mark||grid[i-4][j]!=mark)
                return false;
            else
                i++;
        }
